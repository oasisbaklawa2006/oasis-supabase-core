...
`supabase/config.toml` declares four approved repository-managed functions for branch/preview deployment:
...
`whatsapp-content-interpret` now accepts a chronological packet of authoritative inbound provider message identifiers and re-loads every message under the caller's RLS scope. The preview contract covers:
...
Repository governance locks the interpreter to the canonical Lovable AI Gateway contract:
...
- `whatsapp-content-interpret` is additional repository source for preview/staging certification only and must not be counted as a live production function until approved activation.
...
Production deployment, live secret validation, provider callback verification, controlled bridge dry runs and runtime sign-off remain exclusively within Procedure 8. The 2026-08-17 `whatsapp-content-interpret` staging/preview activation does not constitute production activation.

...
export function inferExpectation(
  ...
  if (
    /\border cancel|cancel order|cancelled order|cancel the order\b/.test(body)
  ) {
  ...
  if (
    /\b(please add|add more|add another|qty increase|make it|revised order|revised qty|revision|increase to|reduce to)\b/
      .test(body)
  ) {
  ...
  if (
    /\b(dispatch today|dispatch tomorrow|urgent dispatch|asap dispatch|dispatch asap|dispatch now|balance goods|client waiting|dod|delivery today|deliver today)\b/
      .test(body)
  ) {
  ...
  if (
    /\b(dispatched|out for delivery|delivered|in transit|lr number|transport)\b/
      .test(body)
  ) {
    return pack(
      "DISPATCH_STATUS",
  ...
  if (
    /\b(short received|shortage|missing item|extra quantity|wrong material|wrong qty|complaint|damaged|broken|decorated stock)\b/
      .test(body)
  ) {
    return pack(
      "COMPLAINT",
  ...
  if (
    /\b(address|location|pincode|pin code|deliver to|ship to|transporter|vehicle|lr\b|truck)\b/
      .test(body)
  ) {
    const cls = /\btransporter|vehicle|lr\b|truck\b/.test(body)
      ? "TRANSPORTER"
      : "DELIVERY_ADDRESS";
    return pack(
      cls,
  ...
  if (
    /\b(pi\b|proforma|pro-forma|invoice copy|ledger|statement)\b/.test(body)
  ) {
    const cls = /\bpi\b|proforma|pro-forma/.test(body)
      ? "PI_REQUEST"
      : "INVOICE_LEDGER";
    return pack(
      cls,
  ...
  if (
    ORDER_LINE_RE.test(body) ||
    /\b(order|send|dispatch|moq|midya|baklawa|kg|box)\b/.test(body)
  ) {
    const contextBodies = contextIndices
      .map((idx) => allMessages.find((m) => m.index === idx))
      .filter(Boolean) as ParsedHistoricalMessage[];
    const hasAdjacentParty = contextBodies.some((m) =>
      m.party_hints.length > 0
    );
  ...
  if (
    /\b(hod|team|shift|production|factory|internal|confirm internally|ops)\b/
      .test(body)
  ) {
    return pack(
      "INTERNAL_OPERATION",
  ...
  if (
    /\b(maybe|not sure|which one|which order|which customer|confirm\??)\b/.test(
      body,
    )
  ) {
    return pack(

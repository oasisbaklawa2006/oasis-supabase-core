...
def main() -> None:
    ...
    if state == "skipped":
        print(
            ...
            "requires an active PR preview branch. Include a governed "
            "supabase/migrations or supabase/config.toml change to trigger Supabase "
            "branching, or run workflow_dispatch "
            "sync-preview-cert-edge-secrets with an explicit preview_project_ref.",
            file=sys.stderr,
        )
        sys.exit(1)
    if state == "failed":

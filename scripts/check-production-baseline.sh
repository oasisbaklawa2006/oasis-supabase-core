...
[[ "$validated_row_count" == '185' ]] || fail "current production ledger must contain exactly 185 versions; found $validated_row_count"
...
[[ "$(find "$archive" -maxdepth 1 -type f -name '*.sql' | wc -l | tr -d ' ')" == '41' ]] || fail "expected 41 superseded Core migrations in the audit archive"

...
async function callRunner(
  ...
  const runnerUrl =
    `${previewUrl.replace(/\/$/, "")}/functions/v1/whatsapp-stage1b-cert-runner`;
...
async function main(): Promise<void> {
  ...
  for (const [index, fixture] of fixtures.entries()) {
    console.error(`Scoring fixture ${index + 1}/${fixtures.length}: ${fixture.id}`);
    ...
    if (!scored.ok) {
      const report: HarnessReport = {
        ...
        status: "FAILED",
        final_verdict: "FAIL",
        blocker: `FIXTURE_${fixture.id}_HTTP_${scored.status}:${String(scored.body.error ?? "")}`,
        preview_project_ref: PREVIEW_CERT_PROJECT_REF,
        run_id: runId,
        results,

...
# --- Supabase CLI -----------------------------------------------------------
case "$(uname -m)" in
  x86_64|amd64) SUPABASE_DEB_ARCH="amd64" ;;
  aarch64|arm64) SUPABASE_DEB_ARCH="arm64" ;;
  *)
    echo "INSTALL: ERROR: unsupported architecture $(uname -m) for Supabase CLI package" >&2
    exit 1
    ;;
esac
...
if command -v supabase >/dev/null 2>&1 && [ "$(supabase --version 2>/dev/null | head -1)" = "${SUPABASE_CLI_VERSION}" ]; then
  ...
  log "installing supabase CLI ${SUPABASE_CLI_VERSION} (${SUPABASE_DEB_ARCH})"
  tmp_deb="$(mktemp --suffix=.deb)"
  curl -fsSL -o "$tmp_deb" \
    "https://github.com/supabase/cli/releases/download/v${SUPABASE_CLI_VERSION}/supabase_${SUPABASE_CLI_VERSION}_linux_${SUPABASE_DEB_ARCH}.deb"
  sudo dpkg -i "$tmp_deb"
  rm -f "$tmp_deb"
fi
...
if [ -d "${repo_root}/supabase" ] && sudo "$(command -v bash)" "$(dirname "$0")/start.sh" >/tmp/install-docker-boot.log 2>&1; then
  if (cd "${repo_root}" && supabase status >/dev/null 2>&1); then
    stack_was_running=true
  fi
  if (cd "${repo_root}" && supabase start >/tmp/install-warm-start.log 2>&1); then

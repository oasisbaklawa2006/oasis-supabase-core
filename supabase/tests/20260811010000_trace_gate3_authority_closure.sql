-- Contract for migration 20260811010000_trace_gate3_authority_closure.
begin;
select plan(16);
...
select has_table('public','ols_trace_mutation_receipts');
select col_is_unique('public','ols_trace_mutation_receipts','idempotency_key');
select has_trigger('public','ols_audit_logs','trg_ols_audit_logs_immutable');
select has_trigger('public','ols_trace_mutation_receipts','trg_ols_trace_receipts_immutable');
...
select is_empty($$select 1 from pg_policies where schemaname='public' and tablename like 'ols\_%' escape '\' and cmd in ('INSERT','UPDATE','DELETE') and 'authenticated'=any(roles)$$,'authenticated direct mutation policies are removed');

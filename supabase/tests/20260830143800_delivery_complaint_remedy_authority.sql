-- Contract for migration 20260830143800_delivery_complaint_remedy_authority.sql.
...
select ok(pg_get_functiondef('public.record_delivery_proof_v1(uuid,timestamp with time zone,text,jsonb,text,text,uuid)'::regprocedure) like '%interval ''10 days''%','delivery proof returns ten-calendar-day deadline');
...
select ok(pg_get_functiondef('public.file_commercial_complaint_v1(uuid,text,text,jsonb,text,text,text,uuid)'::regprocedure) like '%statement_timestamp()>v_deadline%','day-10 boundary uses greater-than, so day 10 is accepted');
...
select ok(pg_get_functiondef('public.file_commercial_complaint_v1(uuid,text,text,jsonb,text,text,text,uuid)'::regprocedure) like '%v_deadline:=NULL%','missing delivery proof does not silently start/expire clock');
...
select ok((select definition from pg_views where schemaname='public' and viewname='commercial_complaint_window_v1') like '%EXPIRED_NO_COMPLAINT%','window derives clean expiry state');

...
select ok(has_function_privilege('service_role','public.capture_whatsapp_commercial_fragment_for_potential(uuid,uuid,integer,boolean,jsonb)'::regprocedure,'EXECUTE'),'service_role can execute bridge');

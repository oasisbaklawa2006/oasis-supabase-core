...
select isnt_empty($$select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='release_carton_at_dispatch_gate_v1' and p.prosecdef and p.proconfig @> array['search_path=pg_catalog, public, auth']$$,'gate governed fixed path');

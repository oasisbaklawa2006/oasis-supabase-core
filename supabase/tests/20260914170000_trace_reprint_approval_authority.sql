-- Contract test for migration 20260914170000_trace_reprint_approval_authority.sql
begin;
select plan(15);
...
insert into public.users (id, role, is_sales_executive) values
  ...
  ('d3140000-0000-0000-0000-000000000003', 'SUPER_ADMIN', false)
...
set local request.jwt.claim.sub = 'd3140000-0000-0000-0000-000000000003';

select is(
  public.trace_reprint_approver_allowed_v1(), false,
  'super admin is not implicitly granted Trace reprint approval authority'
);
...
select is(
  ...
  'true',
  'exact idempotent replay reports replay evidence without mutating stored response'
);
select is(

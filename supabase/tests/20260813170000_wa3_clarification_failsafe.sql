...
select isnt_empty($$select 1 from pg_proc where oid='public.wa2_guard_sales_order_draft_write()'::regprocedure and pg_get_functiondef(oid) ilike '%tg_op%insert%' and pg_get_functiondef(oid) like '%WA2_DRAFT_PROMOTE_REQUIRED%'$$,'support cannot bypass AAL2 promotion through insert');

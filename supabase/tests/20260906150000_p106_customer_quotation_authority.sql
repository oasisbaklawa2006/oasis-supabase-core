-- Contract for 20260906150000_p106_customer_quotation_authority.sql
begin;
...
select ok(
  (select provolatile from pg_proc where oid = 'public.customer_quotation_is_actionable_v1(text,timestamptz)'::regprocedure) = 's',
  'customer_quotation_is_actionable_v1 is STABLE because it evaluates statement_timestamp()'
);

select ok(
  lower(pg_get_functiondef('public.customer_quotation_is_actionable_v1(text,timestamptz)'::regprocedure)) like '%stable%',
  'customer_quotation_is_actionable_v1 definition is explicitly STABLE in undeployed P106 migration'
);

select ok(
  pg_get_functiondef('public.accept_customer_quotation_v1(uuid,integer,text)'::regprocedure)
    not like '%status = ''expired''%'
  and pg_get_functiondef('public.decline_customer_quotation_v1(uuid,integer,text,text)'::regprocedure)
    not like '%status = ''expired''%',
  'accept/decline do not persist rollback-doomed expired status writes'
);

select ok(
  pg_get_functiondef('public.accept_customer_quotation_v1(uuid,integer,text)'::regprocedure)
    like '%customer_quotation_terminal:%'
  and pg_get_functiondef('public.accept_customer_quotation_v1(uuid,integer,text)'::regprocedure)
    like '%FOR UPDATE%'
  and pg_get_functiondef('public.decline_customer_quotation_v1(uuid,integer,text,text)'::regprocedure)
    like '%customer_quotation_terminal:%'
  and pg_get_functiondef('public.decline_customer_quotation_v1(uuid,integer,text,text)'::regprocedure)
    like '%FOR UPDATE%',
  'accept and decline serialize terminal transitions with quotation-level lock and row FOR UPDATE'
);
...
begin
  ...
  if (select q.status from public.customer_quotations q where q.id = v_quotation_id) <> 'declined' then
    raise exception 'REGRESSION: decline did not set status declined';
  end if;

  -- terminal transition serialization: accept then decline with different idempotency keys
  reset role;
  perform set_config('request.jwt.claims', json_build_object('sub', v_buyer_a::text, 'role', 'authenticated')::text, true);

  select q.quotation_id into v_quotation_id
  from public.submit_customer_quotation_request_v1(
    'quote-terminal-serialize-001',
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 10)),
    null
  ) q;
  ...
  perform public.accept_customer_quotation_v1(v_quotation_id, 1, 'accept-serialize-key-a');

  begin
    perform public.decline_customer_quotation_v1(v_quotation_id, 1, 'decline-serialize-key-b', 'Too late');
    raise exception 'REGRESSION: decline after accept was allowed';
  exception
    when others then
      if sqlerrm not like 'QUOTATION_ALREADY_ACCEPTED%' then
        raise;
      end if;
  end;

  if (select q.status from public.customer_quotations q where q.id = v_quotation_id) <> 'accepted' then
    raise exception 'REGRESSION: accept+decline race left quotation non-accepted';
  end if;

  if not exists (
    select 1 from public.customer_quotation_acceptance_handoffs h
    where h.quotation_id = v_quotation_id and h.handoff_status = 'pending'
  ) then
    raise exception 'REGRESSION: accept handoff missing after blocked decline';
  end if;

  if exists (
    select 1 from public.customer_quotation_events e
    where e.quotation_id = v_quotation_id and e.event_type = 'DECLINED'
  ) then
    raise exception 'REGRESSION: decline event recorded after accepted terminal state';
  end if;

  -- terminal transition serialization: decline then accept with different idempotency keys
  reset role;
  perform set_config('request.jwt.claims', json_build_object('sub', v_buyer_a::text, 'role', 'authenticated')::text, true);

  select q.quotation_id into v_quotation_id
  from public.submit_customer_quotation_request_v1(
    'quote-terminal-serialize-002',
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 10)),
    null
  ) q;
  ...
  perform public.decline_customer_quotation_v1(v_quotation_id, 1, 'decline-serialize-key-c', 'No thanks');

  begin
    perform public.accept_customer_quotation_v1(v_quotation_id, 1, 'accept-serialize-key-d');
    raise exception 'REGRESSION: accept after decline was allowed';
  exception
    when others then
      if sqlerrm not like 'QUOTATION_NOT_ACTIONABLE%' then
        raise;
      end if;
  end;

  if (select q.status from public.customer_quotations q where q.id = v_quotation_id) <> 'declined' then
    raise exception 'REGRESSION: decline+accept race left quotation non-declined';
  end if;

  if exists (
    select 1 from public.customer_quotation_acceptance_handoffs h
    where h.quotation_id = v_quotation_id
  ) then
    raise exception 'REGRESSION: accept handoff created after declined terminal state';
  end if;

  -- expiry fail-closed on separate quotation
  reset role;
  perform set_config('request.jwt.claims', json_build_object('sub', v_buyer_a::text, 'role', 'authenticated')::text, true);

  select q.quotation_id into v_quotation_id
  from public.submit_customer_quotation_request_v1(
    'quote-expiry-key-001',
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 10)),
    null
  ) q;

  update public.customer_quotations q
  ...
  if (select q.status from public.customer_quotations q where q.id = v_quotation_id) <> 'issued' then
    raise exception 'REGRESSION: expiry test must start from a fresh issued quotation';
  end if;
  ...
  exception
    when others then
      if sqlerrm not like 'QUOTATION_EXPIRED%' then
        raise;
      end if;
  end;

  if (select q.status from public.customer_quotations q where q.id = v_quotation_id) <> 'issued' then
    raise exception 'REGRESSION: expired accept must not persist expired status mutation';
  end if;

  if exists (
    select 1 from public.customer_quotation_events e
    where e.quotation_id = v_quotation_id and e.event_type = 'EXPIRED'
  ) then
    raise exception 'REGRESSION: expired accept must not claim persisted EXPIRED event';
  end if;

  select q.status into v_derived_status
  from public.customer_quotations_v1() q
  where q.quotation_id = v_quotation_id;

  if v_derived_status <> 'expired' then
    raise exception 'REGRESSION: buyer projection must still derive expired status, got %', v_derived_status;
  end if;

  -- revision idempotency scoped to quotation_id
  reset role;
  perform set_config('request.jwt.claims', json_build_object('sub', v_buyer_a::text, 'role', 'authenticated')::text, true);

  select q.quotation_id into v_quotation_id
  from public.submit_customer_quotation_request_v1(
    'quote-revise-scope-a',
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 10)),
    null
  ) q;

  select q.quotation_id into v_quotation_id_b
  from public.submit_customer_quotation_request_v1(
    'quote-revise-scope-b',
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 10)),
    null
  ) q;
  ...
  perform public.revise_customer_quotation_v1(
    v_quotation_id,
    jsonb_build_array(jsonb_build_object('product_id', v_product, 'quantity', 11)),
    'shared-revise-idempotency-key',
    statement_timestamp() + interval '45 days'
  );
  ...
  begin
    ...
    raise exception 'REGRESSION: cross-quotation revise idempotency key reuse succeeded';
  exception
    when unique_violation then
      null;
    when others then
      if sqlerrm not like '%unique%' and sqlerrm not like '%duplicate key%' then
        raise;
      end if;
  end;

  if (select count(*) from public.customer_quotation_versions v where v.quotation_id = v_quotation_id_b) <> 1 then
    raise exception 'REGRESSION: cross-quotation revise key created version on wrong quotation';
  end if;
  ...
  if (select count(*) from public.customer_quotation_versions v where v.quotation_id = v_quotation_id) <> 2 then
    raise exception 'REGRESSION: same-quotation revise idempotency replay did not preserve version count';
  end if;
...
select pass('quotation RPCs enforce request/list/detail/lines/accept/decline/expiry/version/isolation/idempotency/terminal-serialization/revise-scope');

...
begin;
select plan(5);
select is_empty($$select 1 from information_schema.table_privileges where table_schema='public' and table_name='whatsapp_config' and grantee in('PUBLIC','anon','authenticated')$$,'browser roles have no whatsapp_config grants');
...
select isnt_empty($$select 1 from information_schema.table_privileges where table_schema='public' and table_name='whatsapp_config' and grantee='service_role' and privilege_type='SELECT'$$,'trusted service role retains compatibility read');

...
declare v_gst text; v_addr text; v_code text;
begin
 if p_company_id is null then return ''; end if;
 select lower(btrim(coalesce(gst_number,''))),lower(btrim(coalesce(registered_address,''))) into v_gst,v_addr from public.companies where id=p_company_id;
 if v_gst<>'' then v_code:=left(regexp_replace(v_gst,'\\s','','g'),2); if v_code='07' then return 'delhi'; end if; end if;
 if v_addr~'(^|[, ])delhi([, ]|$)' or v_addr like '%new delhi%' then return 'delhi'; end if; return '';
end $$;
...
begin
 ...
 if not found or v_scan.entity_id is distinct from p_carton_id or v_scan.order_id is distinct from v_carton.order_id or v_scan.entity_type<>'dispatch_carton' or v_scan.scan_type not in('carton','dispatch_gate') or v_scan.verification_status not in('scanned','verified') or lower(btrim(v_scan.barcode_value))<>lower(btrim(v_carton.barcode_string)) then
 ...
 perform set_config('app.dispatch_carton_authority_managed','on',true);

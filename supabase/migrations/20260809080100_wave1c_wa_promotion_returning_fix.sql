...
declare v_draft public.sales_order_drafts%rowtype; v_order_id uuid; v_order_number text; v_line record; v_qty numeric; v_lines int:=0; v_actor_name text;
begin
 ...
 select coalesce(nullif(btrim(u.full_name),''),nullif(btrim(u.name),''),nullif(btrim(u.email),''),p_actor_id::text)
   into v_actor_name from public.users u where u.id=p_actor_id;
 ...
 update public.sales_order_drafts d set status='APPROVED_FOR_SO',promoted_order_id=v_order_id,approver_id=p_actor_id,approver_name=v_actor_name,review_notes=p_review_notes,updated_by=p_actor_id where d.id=p_draft_id;
 insert into public.sales_order_draft_audit_log(draft_id,action,from_status,to_status,actor_id,actor_name,metadata) values(p_draft_id,'APPROVE','UNDER_REVIEW','APPROVED_FOR_SO',p_actor_id,v_actor_name,coalesce(p_metadata,'{}')||jsonb_build_object('promoted_order_id',v_order_id));

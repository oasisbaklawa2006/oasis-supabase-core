...
--
-- Repairs:
--   1) accept Trace's historical `shipping` ref_type while canonicalizing new
--      allocation authority to `shipping_label`;
--   2) make an approval request single-use across durable allocations;
--   3) allow a blocked idempotent allocation to attach an approval request and
--      re-evaluate that SAME allocation after approval, without incrementing the
--      counter or creating a second allocation;
--   4) return fresh allocation state on idempotent replay so an approval
--      transition cannot be masked by the immutable original receipt response.
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'TRACE_REPRINT_APPROVAL_DUPLICATE_REMEDIATION_REQUIRED'
      USING ERRCODE = 'P0001';
  END IF;
END;
$$;

CREATE UNIQUE INDEX IF NOT EXISTS ols_trace_reprint_allocations_approval_request_uniq
...
BEGIN
  ...
  INSERT INTO public.ols_trace_reprint_allocations(
    ...
    v_approval_required,
    v_approval_granted,
    v_effective_approval_request_id,
    v_allowed
  )

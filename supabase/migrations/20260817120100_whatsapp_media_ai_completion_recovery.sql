...
begin
  ...
  update public.whatsapp_commercial_evidence
     set processing_state=p_state,
         processing_detail=coalesce(processing_detail,'{}'::jsonb)
           ...
           || jsonb_build_object('attempt_key',btrim(p_attempt_key)),
         processed_at=now()
   where id=v_row.id
  ...
  values(v_row.packet_id,'MEDIA_PROCESSING_'||p_state,
    jsonb_build_object('evidence_id',v_row.id,'detail',coalesce(p_detail,'{}')));
  ...
  if p_state<>'SUCCEEDED' then
    ...
    -- Older capture-only ingress marked any media presence as a failed
    -- interpretation before the AI worker existed. Recover only when every
    -- remaining failed media evidence row is proven to be that bootstrap marker;
    -- never erase an independent human/AI interpretation failure.
    select not exists (
      select 1
    ...
    if v_only_media_bootstrap_failures and not exists (
      select 1
       ...
       where e.potential_order_id=v_row.potential_order_id
         and e.media_count>0
         and e.processing_state<>'SUCCEEDED'
    ) then
      perform set_config('app.wa1_governed_mutation','on',true);
      update public.whatsapp_potential_orders

...
begin
 if current_setting('app.dispatch_carton_authority_managed',true)='on' or current_user in ('postgres','service_role') then return new; end if;
...
begin
 ...
 select coalesce(sum(amount),0) into v_verified from public.order_payments where order_id=p_order_id and status='verified';
...
declare v_reason text:=btrim(coalesce(p_rejection_reason,''));
begin
 ...
 perform 1 from public.orders where id=p_order_id for update; if not found then return jsonb_build_object('ok',false,'blockers',jsonb_build_array(jsonb_build_object('code','not_found'))); end if;
 perform set_config('app.order_authority_managed','on',true);
...
begin
 ...
 v_payment:=lower(btrim(coalesce(p_payment_status,v.payment_status,'')));
 if v.status not in('awaiting_advance','confirmed','submitted','awaiting_payment','awaiting_final_payment') or not public.is_advance_verification_path_cleared(v_payment) or coalesce(v.advance_paid,0)<coalesce(v.advance_required,0) then return jsonb_build_object('ok',false,'blockers',jsonb_build_array(jsonb_build_object('code','finance_not_cleared'))); end if;
 perform set_config('app.order_authority_managed','on',true); update public.orders set status='manufacturing',payment_status=v_payment,finance_verified_by=auth.uid(),finance_verified_at=now() where id=p_order_id;
...
begin
 ...
 perform set_config('app.order_authority_managed','on',true); update public.orders set status='in_production',payment_status=v_payment,finance_verified_by=auth.uid(),finance_verified_at=now() where id=p_order_id;
...
begin
 ...
 perform set_config('app.order_authority_managed','on',true); update public.orders set status='packed_ready' where id=p_order_id;
...
begin
 ...
 perform set_config('app.order_authority_managed','on',true); update public.orders set status='cleared_for_dispatch' where id=p_order_id;

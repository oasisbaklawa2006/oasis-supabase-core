...
-- Durable, privacy-preserving guard for the public MSG91 widget endpoint.
...
CREATE TABLE public.msg91_otp_security_events (
  ...
  event_type text NOT NULL CHECK (event_type IN ('ATTEMPT','VERIFIED')),
  token_digest text,
  phone_digest text,
...
COMMENT ON TABLE public.msg91_otp_security_events IS
  'Private MSG91 widget rate/replay ledger. Stores SHA-256 digests only; writes are governed by service-role-only RPCs.';
...
BEGIN
  ...
  -- Serialize the per-IP window so concurrent requests cannot race the limit.
...
BEGIN
  ...
  -- The token lock plus unique partial index makes replay rejection atomic.
  ...
  IF EXISTS (
    ...
    WHERE e.event_type = 'VERIFIED'
      AND e.token_digest = p_token_digest
  ) THEN

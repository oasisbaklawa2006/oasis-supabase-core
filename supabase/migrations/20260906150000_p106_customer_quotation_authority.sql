...
--
-- Stacks strictly after CORE-HIERARCHY-MAP-01 (20260906140000).
...
COMMENT ON FUNCTION public.customer_quotation_is_actionable_v1(text, timestamptz) IS
  'Derived buyer-actionability: issued status and not past expires_at. STABLE because it evaluates statement_timestamp(); expiry remains a read-time derived projection when persisted status is still issued.';
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(b) ORDER BY b.product_id), '[]'::jsonb)
  INTO v_built_lines
  FROM public.customer_build_quotation_lines_v1(v_company_id, p_lines) b;

  FOR v_line IN SELECT value FROM jsonb_array_elements(v_built_lines)
  LOOP
  ...
  SELECT
    ...
    elem->>'currency',
    elem->>'uom',
    (elem->>'gst_rate')::numeric,
    (elem->>'tax_inclusive')::boolean,
    (elem->>'minimum_order_quantity')::numeric,
    (elem->>'order_increment')::numeric,
    (elem->>'min_carton_qty')::numeric,
    elem->>'sku',
    elem->>'product_name'
  FROM jsonb_array_elements(v_built_lines) elem;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE v.idempotency_key = btrim(p_idempotency_key)
      AND v.quotation_id = p_quotation_id
  ) THEN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(b) ORDER BY b.product_id), '[]'::jsonb)
  INTO v_built_lines
  FROM public.customer_build_quotation_lines_v1(v_row.company_id, p_lines) b;
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtextextended('customer_quotation_terminal:' || p_quotation_id::text, 0)
  );
  ...
  IF FOUND THEN
    SELECT v.version_number
      INTO v_replay_version
      FROM public.customer_quotation_versions v
     WHERE v.id = v_existing.quotation_version_id;
...
BEGIN
  ...
  IF v_row.status = 'declined' THEN
    RETURN QUERY SELECT p_quotation_id, 'declined'::text, p_version_number, true;
    RETURN;
  END IF;

...
-- b2b_inventory_receipts -> record -> accept -> putaway/GRN stock authority.
--
-- This migration deliberately does NOT create a second stock ledger or a
-- second acceptance subsystem. Existing inventory receipt, disposition and
-- GRN authorities remain canonical.

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';

-- Preserve the already-validated canonical source-reference constraint shipped
-- by the 3PGS procurement repair. Add only the stronger Assembly-specific
-- lineage rule instead of dropping/recreating the existing constraint.
ALTER TABLE public.b2b_inventory_receipts
  DROP CONSTRAINT IF EXISTS b2b_inventory_receipts_assembly_handover_reference_check;

ALTER TABLE public.b2b_inventory_receipts
  ADD CONSTRAINT b2b_inventory_receipts_assembly_handover_reference_check CHECK (
...
COMMENT ON FUNCTION public.create_b2b_inventory_receipt(text, text, text, text, text, jsonb, text, uuid, uuid, text) IS
  'Opens generic governed inward receipts. return_from_assembly must use create_b2b_inventory_receipt_from_assembly_handover so lineage is server-derived.';
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.b2b_inventory_stores s
    WHERE s.store_code = v_handover.destination_reference AND s.active
  ) THEN
    RAISE EXCEPTION 'Assembly handover destination % is not an active canonical inventory store',
      v_handover.destination_reference;
  END IF;
  IF NOT public.can_access_b2b_inventory_store(v_actor_id, v_handover.destination_reference, 'receive') THEN
  ...
  IF FOUND THEN
    ...
    FROM public.b2b_inventory_receipt_lines l WHERE l.receipt_id = v_receipt.id;
  ...
  INSERT INTO public.b2b_inventory_receipts (
    receipt_number, receipt_source, destination_store_code, source_document_type,
    source_document_reference, status, correlation_id, notes
  ) VALUES (
    p_receipt_number, 'return_from_assembly', v_handover.destination_reference,
    'b2b_assembly_handover', p_handover_id::text, 'expected', p_correlation_id, p_notes
  ) RETURNING * INTO v_receipt;

  INSERT INTO public.b2b_inventory_receipt_lines (
    receipt_id, product_id, sku, expected_qty, notes
  ) VALUES (
    v_receipt.id, v_job.output_product_id, v_job.output_sku, p_expected_qty,
...
COMMENT ON FUNCTION public.create_b2b_inventory_receipt_from_assembly_handover(uuid, text, numeric, text, text) IS
  'Binds acknowledged Assembly custody to the existing return_from_assembly receipt subsystem. Product/SKU/destination are server-derived and cumulative physical receipt cannot exceed acknowledged custody.';
...
-- Guard the existing physical-record boundary against over-receipt. This does
-- not post stock; it only ensures a return_from_assembly receipt cannot record
-- more physical quantity than the quantity reserved from the handover.
CREATE OR REPLACE FUNCTION public.guard_b2b_assembly_return_receipt_quantity()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
BEGIN
  ...
  IF v_source = 'return_from_assembly'
     AND coalesce(NEW.received_qty, 0) > NEW.expected_qty THEN
    RAISE EXCEPTION 'Assembly return received quantity % exceeds bound expected quantity %',
...
DROP TRIGGER IF EXISTS trg_guard_b2b_assembly_return_receipt_quantity
  ON public.b2b_inventory_receipt_lines;
CREATE TRIGGER trg_guard_b2b_assembly_return_receipt_quantity
BEFORE INSERT OR UPDATE OF received_qty, expected_qty, receipt_id
ON public.b2b_inventory_receipt_lines
FOR EACH ROW EXECUTE FUNCTION public.guard_b2b_assembly_return_receipt_quantity();

REVOKE ALL ON FUNCTION public.guard_b2b_assembly_return_receipt_quantity() FROM PUBLIC, anon, authenticated;

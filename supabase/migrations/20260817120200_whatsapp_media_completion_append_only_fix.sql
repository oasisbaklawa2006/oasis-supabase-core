...
begin
  ...
  -- Source evidence is immutable. Every processing outcome is appended to the
  -- dedicated event ledger and replayed idempotently by (evidence_id,attempt_key).
  ...
  -- The capture-only webhook historically marks media as UNREADABLE before the
  -- asynchronous interpreter has had a chance to run. That immutable marker is
  -- identifiable only by fail_open_media_review=true. Never recover a genuine
  -- media failure lacking that marker.
  select
    count(*) filter (

...
set source_application = coalesce(source_application, 'legacy'),
    ...
    payload_fingerprint = coalesce(
      payload_fingerprint,
      md5(concat_ws('|', event_type, entity_type, entity_id::text, correlation_id, metadata::text))
    )
...
create unique index if not exists operational_events_source_idempotency_uidx
  on public.operational_events (source_application, idempotency_key)
  where idempotency_key is not null;

create index if not exists operational_events_correlation_idx
  on public.operational_events (correlation_id, occurred_at desc);

create index if not exists operational_events_causation_idx
  on public.operational_events (causation_id)
  where causation_id is not null;

create index if not exists operational_events_entity_timeline_idx
  on public.operational_events (entity_type, entity_id, occurred_at desc);
...
create or replace function public.append_operational_event_v1(
  ...
  p_actor_department text default null,
...
begin
  if auth.role() <> 'service_role' then
    ...
    if not public.is_internal_staff(auth.uid()) then
      raise exception 'internal staff authority required';
    end if;
  end if;
  ...
  if nullif(btrim(p_event_type), '') is null
     ...
     or nullif(btrim(p_source_application), '') is null
  ...
  if p_event_version < 1 then
    raise exception 'event_version must be positive';
  end if;
  ...
  v_fingerprint := md5(concat_ws('|',
    ...
    coalesce(p_queue_item_id::text, ''),
    coalesce(v_actor_id::text, ''),
    coalesce(p_visibility, ''),
    coalesce(p_severity, ''),
    coalesce(p_command_name, ''),
    coalesce(p_command_id, ''),
    coalesce(p_causation_id, ''),
  ...
  begin
    insert into public.operational_events (
      event_type, entity_type, entity_id, order_id, customer_id, queue_item_id,
      actor_id, actor_role, actor_department, visibility, severity, title,
      message, reason_code, reason_text, metadata, correlation_id,
      idempotency_key, source_application, event_version, command_name,
      command_id, causation_id, occurred_at, payload_fingerprint
    ) values (
      p_event_type, p_entity_type, p_entity_id, p_order_id, p_customer_id, p_queue_item_id,
      v_actor_id, p_actor_role, p_actor_department, p_visibility, p_severity, p_title,
      p_message, p_reason_code, p_reason_text, coalesce(p_metadata, '{}'::jsonb), p_correlation_id,
      p_idempotency_key, p_source_application, p_event_version, p_command_name,
      p_command_id, p_causation_id, p_occurred_at, v_fingerprint
  ...
  exception when unique_violation then
    ...
    if v_existing_id is null or v_existing_fingerprint <> v_fingerprint then
      raise exception 'idempotency key conflict';
    end if;
...
revoke all on function public.append_operational_event_v1(
  text,text,uuid,text,text,text,jsonb,uuid,uuid,uuid,uuid,text,text,text,text,text,text,text,text,integer,text,text,text,timestamptz
) from public, anon;

grant execute on function public.append_operational_event_v1(
  text,text,uuid,text,text,text,jsonb,uuid,uuid,uuid,uuid,text,text,text,text,text,text,text,text,integer,text,text,text,timestamptz
) to authenticated, service_role;

comment on function public.append_operational_event_v1(
  text,text,uuid,text,text,text,jsonb,uuid,uuid,uuid,uuid,text,text,text,text,text,text,text,text,integer,text,text,text,timestamptz

-- Finance Exit review hardening for 20260831101500 / 20260831101600.
...
BEGIN
  ...
  -- Fail closed if historical/invalid data contains more than one current
  -- submitted DPL for a consignment. Do not multiply lines or cartons.
  IF EXISTS(
    SELECT 1
    FROM public.b2b_dispatch_consignments c
    JOIN public.b2b_dispatch_packing_list_versions d ON d.consignment_id=c.id
    WHERE c.order_id=p_order_id AND c.status<>'cancelled'
      AND d.superseded_by IS NULL AND d.status='submitted_to_finance'
    GROUP BY c.id
    HAVING count(*)<>1
  ) THEN
    RAISE EXCEPTION 'B2B_FINANCE_DPL_CURRENT_VERSION_AMBIGUOUS' USING ERRCODE='55000';
  END IF;
  ...
  WITH submitted AS (
    ...
    SELECT DISTINCT bc.id carton_id
...
BEGIN
  ...
  IF v_carton.status NOT IN('ready_to_load','loaded') THEN
    v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','carton_not_ready_for_gate','status',v_carton.status));
  END IF;
  IF v_carton.open_photo_ref IS NULL OR v_carton.net_weight IS NULL OR v_carton.gross_weight IS NULL
    ...
    v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','carton_lock_evidence_incomplete'));
  END IF;
  ...
  END IF;
  UPDATE public.b2b_dispatch_cartons SET status='handed_over',physical_location='LOADED' WHERE id=p_carton_id;
...
-- This projection is user-session internal staff authority, not backend service authority.

...
Deno.test("approval notification uses authoritative approved application data", () => {
  ...
  assertEquals(approvalChannels(notification), ["email", "whatsapp"], "channel fanout");
  if (!notification.message.includes("CERT B2B CO") || !notification.message.includes("GOLD")) {
    throw new Error("approval message must contain authoritative business/tier context");
  }
});
...
Deno.test("unapproved application is fail closed", () => {
  ...
  try {
    ...
    failed = error instanceof Error && error.message === "application_not_approved";
  }
  if (!failed) throw new Error("pending application must not produce approval notification");
});

Deno.test("channel identity is deterministic and retry safe", () => {
  const id = "10000000-0000-4000-8000-000000000003";
  assertEquals(approvalIdempotencyKey(id, "email"), `b2b-access-approved:${id}:email`, "email key");

...
Deno.test("direct Gemini provider uses current frozen model and generateContent endpoint", () => {
  ...
  assertStringIncludes(GEMINI_GENERATE_CONTENT_URL, "/v1beta/models/gemini-3.7-flash:generateContent");
});
...
Deno.test("request construction preserves text and inline image/audio/video/pdf evidence", () => {
  ...
  assertEquals(request.generationConfig.temperature, 0);
...
Deno.test("Gemini response parser joins textual candidate parts", () => {
  assertEquals(parseGeminiJsonText({
...
Deno.test("direct provider sends x-goog-api-key and parses JSON text", async () => {
  ...
  const fakeFetch: typeof fetch = (input, init) => {
    ...
    return Promise.resolve(new Response(JSON.stringify({
      candidates: [{ content: { parts: [{ text: "{\"intent\":\"OTHER\"}" }] } }],
  ...
  assertEquals(new Headers(capturedInit?.headers).get("x-goog-api-key"), "preview-key");
});

Deno.test("provider HTTP failure is explicit and has no fallback", async () => {
  const fakeFetch: typeof fetch = () => Promise.resolve(new Response("no", { status: 429 }));
  await assertRejects(
    () => callGeminiGenerateContent("key", buildGeminiRequest([textPart("x")]), fakeFetch),
    Error,
...
Deno.test("provider malformed response fails closed", async () => {
  const fakeFetch: typeof fetch = () => Promise.resolve(new Response("not-json", { status: 200 }));
  await assertRejects(
    () => callGeminiGenerateContent("key", buildGeminiRequest([textPart("x")]), fakeFetch),
    Error,

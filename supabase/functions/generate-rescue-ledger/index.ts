...
function storagePathFromRef(value: string | null): string | null {
  ...
  if (value.startsWith(canonicalPrefix)) {
    return value.slice(canonicalPrefix.length);
  }
  ...
  try {
    return decodeURIComponent(
      value.slice(markerIndex + legacyMarker.length).split("?")[0],
    );
  } catch {
    return null;
  }
...
async function createLedgerSignedUrl(
  ...
  if (!path) return null;
  const { data, error } = await admin.storage.from(LEDGER_BUCKET)
    .createSignedUrl(path, SIGNED_LEDGER_URL_TTL_SECONDS);
  if (error || !data?.signedUrl) {
    console.error(
      "[generate-rescue-ledger] signed URL creation failed",
...
async function buildRescuePdf(args: {
  ...
  page.drawText("Continuity Period Statement", {
    x: 40,
    y: 770,
    size: 11,
    font: helv,
  ...
  page.drawText(args.businessName, {
    x: 40,
    y: 720,
    size: 13,
    font: bold,
  ...
  if (args.settlementDeadline) {
    page.drawText(`Target: ${fmtDate(args.settlementDeadline)}`, {
      x: 400,
      y: 790,
      size: 9,
      font: helv,
  ...
  page.drawText(`Remaining rescue balance: ${fmtINR(args.rescueBalance)}`, {
    x: 40,
    y: 680,
    size: 11,
    font: bold,
  ...
  let y = 640;
  page.drawText("CONTINUITY-PERIOD ORDER VALUES", {
    x: 40,
    y,
    size: 9,
    font: bold,
  ...
  for (const row of args.accruedRows) {
    if (y < 130) {
      page.drawText(
        "(additional orders not shown; full detail on the portal)",
        { x: 40, y: 118, size: 8, font: helv, color: mute },
      );
      break;
    }
    page.drawText(fmtDate(row.date), {
      x: 50,
      y,
      size: 9,
      font: helv,
    ...
    page.drawText(fmtINR(row.amount), {
      x: 460,
      y,
      size: 9,
      font: bold,
  ...
  page.drawText(`Accrued order value: ${fmtINR(args.accruedTotal)}`, {
    x: 40,
    y: 100,
    size: 9,
    font: helv,
  ...
  page.drawText(`CURRENT TOTAL OUTSTANDING: ${fmtINR(args.totalDue)}`, {
    x: 40,
    y: 78,
    size: 12,
    font: bold,
...
async function sendSoftWhatsApp(
  ...
  const to = to91(phone);
  if (!/^91\d{10}$/.test(to)) {
    return { ok: false, id: null, error: "invalid_phone" };
  }
  ...
  try {
    const response = await fetch("https://crm.click2api.in/api/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        document: {
          link: pdfUrl,
          filename: "Oasis-Continuity-Statement.pdf",
          caption: message,
        },
      }),
    });
    ...
    if (response.ok) {
      return {
        ok: true,
        id: payload?.messages?.[0]?.id || payload?.id || "sent",
        error: null,
      };
    }
    ...
    console.warn(
      "[generate-rescue-ledger] document delivery outcome uncertain",
  ...
  try {
    ...
    console.warn(
      "[generate-rescue-ledger] fallback delivery outcome uncertain",
      error instanceof Error ? error.name : "unknown",
    );
...
async function deliverLedger(
  ...
  if (!ledger.pdf_url) {
    ...
    return {
      ok: false,
      delivery_status: "failed",
      error: "pdf_reference_unavailable",
    };
  }
  ...
  const delivery = await sendSoftWhatsApp(
    phone,
    businessName,
    totalDue,
    deadline,
  ...
  if (delivery.ok) {
    const { error: finalizeError } = await admin.from("bi_monthly_ledgers")
      .update({
    ...
    if (finalizeError) {
      console.error(
        "[generate-rescue-ledger] sent delivery could not be finalized",
  ...
  if (String(delivery.error || "").includes("uncertain")) {
    await admin.from("bi_monthly_ledgers").update({
      last_delivery_error: delivery.error,
    }).eq("id", ledger.id);
    return {
      ok: false,
      delivery_status: "sending",
  ...
  const { error: failError } = await admin.from("bi_monthly_ledgers").update({
    delivery_status: "failed",
    delivery_lease_until: null,
  ...
  return {
    ok: false,
    delivery_status: "failed",
    error: delivery.error || "provider_failed",
  };
}
...
serve(async (req) => {
  ...
  const authority = await requireFinancialLedgerAuthority(
    req,
    supabaseUrl,
    serviceRoleKey,
    publicKey,
  ...
  try {
    ...
    let query = admin.from("companies").select(
      "id, business_name, phone, total_outstanding, rescue_payment_date, settlement_deadline",
    ).eq("payment_terms", "credit");
    query = body.company_id
      ? query.eq("id", body.company_id)
      : query.gt("total_outstanding", 0);
    ...
    const targets = (companies || []).filter((company: any) =>
      ...
      (company.rescue_payment_date &&
        company.rescue_payment_date >= startOfMonth)
    );
    ...
    for (const company of targets) {
      ...
      const periodEnd = company.settlement_deadline
        ? String(company.settlement_deadline).slice(0, 10)
        : fallbackMonthEnd;
      const { data: existing, error: existingError } = await admin.from(
        "bi_monthly_ledgers",
      )
        .select(
          "id, pdf_url, whatsapp_message_id, delivery_status, delivery_attempt_count",
        )
        .eq("company_id", company.id).eq("period_start", periodStart).eq(
          "period_end",
          periodEnd,
        ).eq("ledger_kind", LEDGER_KIND).maybeSingle();
      ...
      const { data: orders, error: ordersError } = await admin.from("orders")
        ...
        .eq("company_id", company.id).gte("created_at", anchor).not(
          "status",
          "in",
      ...
      const accruedTotal = accruedRows.reduce(
      ...
      const { error: uploadError } = await admin.storage.from(LEDGER_BUCKET)
      ...
      results.push({
        ...
        accrued_order_value: accruedTotal,
    ...
    console.error(
      "[generate-rescue-ledger] failure",

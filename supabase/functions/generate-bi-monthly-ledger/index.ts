...
async function createLedgerSignedUrl(
  ...
  if (error || !data?.signedUrl) {
    console.error(
      "[generate-bi-monthly-ledger] signed URL creation failed",
...
async function buildLedgerPdf(args: {
  ...
  page.drawText("Bi-Monthly Account Statement", {
    x: 40,
    y: 770,
    size: 11,
    font: helv,
  ...
  page.drawText(
    "no action is needed. If you spot any discrepancy, kindly reply 'Request Correction' on WhatsApp.",
  ...
  if (args.rows.length === 0) {
    page.drawText("No orders in this period.", {
      x: 50,
      y,
      size: 10,
      font: helv,
  ...
  page.drawText("CURRENT TOTAL OUTSTANDING", {
    x: 50,
    y: y - 12,
    size: 10,
    font: bold,
  ...
  page.drawText(fmtINR(args.totalOutstanding), {
    x: 460,
    y: y - 12,
    size: 12,
    font: bold,
  ...
  page.drawText(
    "Thank you for your continued partnership with Oasis Baklawa.",
...
async function sendWhatsAppPdf(
  ...
  try {
    const response = await fetch("https://crm.click2api.in/api/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        document: {
          link: pdfUrl,
          filename: "Oasis-Account-Statement.pdf",
          caption: message,
        },
      }),
    });
    ...
    // Only a definite provider rejection makes a second text send safe.
  } catch (error) {
    console.warn(
      "[generate-bi-monthly-ledger] document delivery outcome uncertain",
  ...
  try {
    ...
    console.warn(
      "[generate-bi-monthly-ledger] fallback delivery outcome uncertain",
      error instanceof Error ? error.name : "unknown",
    );
...
async function deliverLedger(
  ...
  if (claimError) {
    console.error(
      "[generate-bi-monthly-ledger] delivery claim failed",
      claimError.message,
    );
    return {
      ok: false,
  ...
  if (delivery.ok) {
    ...
    if (finalizeError) {
      console.error(
        "[generate-bi-monthly-ledger] sent delivery could not be finalized",
  ...
  if (failError) {
    console.error(
      "[generate-bi-monthly-ledger] failed delivery could not be finalized",
      failError.message,
    );
    return {
      ok: false,
...
serve(async (req) => {
  ...
  if (
    (body.period_end && !requestedEnd) || (body.period_start && !requestedStart)
  ...
  try {
    ...
    if (body.company_id) {
      const { data, error } = await admin.from("companies").select(
        "id, business_name, phone, payment_terms, total_outstanding",
    ...
    for (const company of companies) {
      const { data: existing, error: existingError } = await admin.from(
        ...
        .eq("company_id", company.id).eq("period_start", periodStartIso).eq(
          "period_end",
          periodEndIso,
      ...
      const { data: orders, error: ordersError } = await admin.from("orders")
        .select("id, status, sales_order_value, created_at").eq(
          "company_id",
          company.id,
        )
        .gte("created_at", `${periodStartIso}T00:00:00+05:30`).lte(
          "created_at",
          `${periodEndIso}T23:59:59.999999+05:30`,
        )
        .not("status", "in", "(draft,cart,cancelled)").order("created_at", {
          ascending: true,
        });
      ...
      const path =
        `ledgers/${company.id}/${periodStartIso}_${periodEndIso}.pdf`;
    ...
    console.error(
      "[generate-bi-monthly-ledger] failure",
